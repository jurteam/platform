...
const TakeOathButton = ({ isOpen, open, close, type }) => {
  return (
    <>
      <Button color={color(type)} size={size(type)} onClick={open}>
        {label(type)}
      </Button>
      <TakeOathModal isOpen={isOpen} onRequestClose={close} />
    </>
  );
};
...
const mapStateToProps = state => ({ isOpen: state.oathKeeper.isModalOpen });
