...
export const ContractSetPenaltyFee = props => {
  ...
  const hasReallyPenaltyFee = () => {
    return (
      penaltyFee !== null &&
      (penaltyFee.partA !== "0.000000000000000000" ||
        penaltyFee.partB !== "0.000000000000000000")
    );
  };
  ...
  return (
    <div className="jur-contract-set-penalty-fee">
      <div className="jur-contract-set-penalty-fee__title">
        <BlockTitle
          title={labels.isThereAnyPenaltyFee}
          description={labels.penaltyFeeDescription}
        />
        <Switch
          onChange={handlePenaltyFeeActive}
          checked={isActive && contract.amount > 0}
          disabled={disabled}
        />
      </div>
      <div
        className={`jur-contract-set-penalty-fee__values ${
          isActive && !disabled && contract.amount > 0
            ? "jur-contract-set-penalty-fee__values--active"
            : ""
        }`}
      >
        {[from, to].map((counterparty, index) =>
          contract.penaltyFee && isActive ? (
            <PriceRange
              key={index}
              min={0}
              defaultValue={
                Number(contract.penaltyFee[counterparty.label]) <=
                contract.amount
                  ? contract.penaltyFee &&
                    Number(contract.penaltyFee[counterparty.label])
                  : contract.amount
