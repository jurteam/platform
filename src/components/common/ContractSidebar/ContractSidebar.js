...
export const ContractSidebar = ({
  ...
  const isPartB = currentWallet.address.toLowerCase() === to.wallet.toLowerCase() ? true : false;
  ...
  const availableActions = () => {
    ...
    if (statusId === 1 && isPartB) {
      ...
      return (
        <>
          <Button color="dispute" onClick={onReject}>{labels.reject}</Button>
          <Button color="muted" variant="gradient" onClick={onAccept}>{labels.accept}</Button>
        </>
      )
    }
    ...
    if (statusId === 2) {
      ...
      return (
        <>
          <Button color="gradient" variant="gradient" onClick={onPay} fullWidth>{labels.payment}</Button>
        </>
      )
    }
    ...
    if (statusId === 5) {
      ...
      if (showProposalForm) {
        ...
        return (
          <>
            {!isPartB && <Button color="friendly" variant={(showProposalForm === true) ? "contained" : "outlined"} onClick={() => setShowProposalForm(true)}>{labels.friendlyResolution}</Button>}
            <Button color="dispute" variant={(isPartB || showProposalForm === 2) ? "contained" : "outlined"} fullWidth={isPartB} onClick={() => setShowProposalForm(2)}>{labels.dispute}</Button>
          </>
        )
      }
      ...
      return (
        <>
          <Button color="dispute" fullWidth={isPartB} onClick={() => setShowProposalForm(isPartB ? 2 : true)}>{labels.dispute}</Button>
          {!isPartB && <Button color="success" onClick={onSuccess}>{labels.success}</Button>}
        </>
      )
    }

    const canSave = (disabled || submitDisabled || !formUpdated) ? false : true;
    const canSend = (disabled || !isValid || !currentUserCanPay || formUpdated) ? false : true;

    // default actions
    return (
      <>
        <Button color={!canSave ? "muted" : null} onClick={canSave ? onSubmit : undefined}>{labels.saveContract}</Button>
        <Button color={!canSend ? "muted" : null} variant={!canSend ? "contained" : "gradient"} onClick={canSend ? onSend : undefined}>{labels.sendToCounterparty}</Button>
      </>
    )
  }
  ...
  return (
    <div>
      ...
      <ContractSetValue contract={contract} feeToPay={feeToPay} onChange={onChangeValue} currentUserCanPay={currentUserCanPay} disabled={disabled} showFeeMsg={showFeeMsg} hasError={hasError} />
      ...
      {statusId >= 4 && statusId <= 9 && showProposalForm &&
        <ProposalForm
          ...
          description={(showProposalForm === 2) ? labels.disputeProposalDescription : labels.friendlyProposalDescription}
