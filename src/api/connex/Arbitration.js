...
import { ADD_TRANSACTION } from "../../reducers/types";
...
export default class connexArbitrationContract {
  ...
  async diputeDispersal(winner, sender) {
    log(
      "disputeDispersal - ArbitrationFactoryABI - winner and sender",
      winner,
      sender
    );
  ...
  async agree(account, contractId) {
    ...
    const result = await signingService
      .request([
        {
          ...agreeClause
        }
  ...
  async withdrawDispersal(account, contractId) {
    ...
    const withdrawDispersalMethod = this.thorAccount.method(
      withdrawDispersalABI
    );
  ...
  async amendDisputeDispersal(dispersal, account, contractId) {
    ...
    log(
      "amendDisputeDispersal - amendDisputeDispersalABI",
      amendDisputeDispersalABI
    );

    const amendDisputeDispersalMethod = this.thorAccount.method(
      amendDisputeDispersalABI
    );

    log(
      "amendDisputeDispersal - amendDisputeDispersalMethod",
      amendDisputeDispersalMethod
    );
    ...
    const amendDisputeDispersalClause = amendDisputeDispersalMethod.asClause(
      dispersal
    );

    log(
      "amendDisputeDispersal - amendDisputeDispersalClause",
