...
# 1. nginx + php install
RUN apt update  && \
    apt install -y ufw curl vim zip unzip nano systemd cron supervisor php-fpm php-mysql php-mbstring php-xml php-cli php-zip php-xmlrpc php-soap php-curl php-gd php-bcmath nginx
...
ADD rest /var/www/html
# ADD rest/artisan /var/www/html/artisan
# ADD rest/bootstrap /var/www/html/bootstrap
# ADD rest/composer.json /var/www/html/composer.json # needed for `composer install`
